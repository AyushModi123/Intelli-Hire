...
const ApplicantDetails = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://intelli-hire.onrender.com/job/${id}`
        );
  ...
  return (
    <>
      {jobexist && (
        <div>
          <CandidateJobDescriptionCard
            job_desc={job_desc}
            jobTitle={jobTitle}
          />
          <h1 style={{ color: "white", paddingTop: "60px" }}>
            Applicant Details
          </h1>
          <form className="candidate-form" onSubmit={handleSubmit}>
            ...
            <label className="candidate-label" htmlFor="email">
              Email:
            </label>
            <input
              className="candidate-input"
            ...
            <label className="candidate-label" htmlFor="phone">
              Phone Number:
            </label>
            <input
              className="candidate-input"
