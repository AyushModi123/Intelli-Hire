...
class scraper:
    ...
    def scrape_coding_profiles(self, gfg=None, cf=None, cc=None):
        courseData = asyncio.get_event_loop().run_until_complete(self._scrape(gfg, cf, cc))
        return courseData
    async def _scrape(self, gfg=None, cf=None, cc=None):
...
def scrapeandcleanData():
    codingData = scraper().scrape_coding_profiles()
    cleaned_codingData = data_cleaning().clean_data(codingData)
    # print(cleaned_codingData)
    return cleaned_codingData
